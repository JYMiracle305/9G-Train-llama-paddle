...
def convert_data_to_id(tokenizer: LlamaTokenizer, data: Any):
    # ids = tokenizer.encode(data["input"] + data["output"])
    ...
    #for k in data.keys():
    #    print(k)
    input_ids = tokenizer.encode(data["input"], add_special_tokens=False)
...
class _MixedDatasetBatchPacker:
    ...
    def apply_transform(
        ...
        if transform is None:
            #print("run 000000000000000000000")
            return data
...
class MixedDataset:
    ...
    def state_dict(self):
        ...
        for k, v in states.items():
            ...
            gpu_num_unused_block = paddle.to_tensor([num_unused_block], dtype=paddle.int64).cuda()
            max_unused_blocks = bmt.distributed.all_reduce(gpu_num_unused_block, op="max").cpu().item()
            if max_unused_blocks == 0:
                max_unused_blocks = 1
            gpu_states = paddle.full((max_unused_blocks,), -1, dtype=paddle.int64).cuda()
    ...
    def get(self) -> LlamaBatch:
        ret: LlamaBatch = self._q_data.get(timeout=300)  # type: ignore

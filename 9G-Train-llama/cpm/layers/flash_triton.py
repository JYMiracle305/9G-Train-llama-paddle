...
def _flash_attn_forward(q, k, v, bias=None, causal=False, softmax_scale=None):
    ...
    bias_type = "none"
    if has_bias:
        assert bias.dtype in [q.dtype, paddle.float]
        assert bias.is_cuda
        assert bias.dim() == 4
        if bias.stride(-1) != 1:
    ...
    lse = paddle.empty((batch, nheads, seqlen_q_rounded), device=q.device, dtype=paddle.float32)
    tmp = paddle.empty((batch, nheads, seqlen_q_rounded), device=q.device, dtype=paddle.float32)
...
def _flash_attn_backward(do, q, k, v, o, lse, dq, dk, dv, bias=None, causal=False, softmax_scale=None):
    ...
    softmax_scale = softmax_scale or 1.0 / math.sqrt(d)
    # dq_accum = paddle.zeros_like(q, dtype=paddle.float32)
    dq_accum = paddle.empty_like(q, dtype=paddle.float32)
    delta = paddle.empty_like(lse)
    # delta = paddle.zeros_like(lse)
...
class FlashAttnQKVPackedFunc(paddle.autograd.PyLayer):
    @staticmethod
    ...
    @staticmethod
    def backward(ctx, do):
        qkv, o, lse, bias = ctx.saved_tensor()
        assert not ctx.needs_input_grad[1], "FlashAttention does not support bias gradient yet"
        # Triton's autotune causes the Tensor._version to change, and so Pypaddle autograd
        # does a memcpy. To avoid this we run in inference_mode, which doesn't track the version.
        with paddle.inference_mode():
            dqkv = paddle.empty_like(qkv)
...
class FlashAttnKVPackedFunc(paddle.autograd.PyLayer):
    @staticmethod
    ...
    @staticmethod
    def backward(ctx, do):
        q, kv, o, lse, bias = ctx.saved_tensor()
        assert not ctx.needs_input_grad[2], "FlashAttention does not support bias gradient yet"
        # Triton's autotune causes the Tensor._version to change, and so Pypaddle autograd
        # does a memcpy. To avoid this we run in inference_mode, which doesn't track the version.
        with paddle.inference_mode():
            dq = paddle.empty_like(q)
            dkv = paddle.empty_like(kv)
            _flash_attn_backward(
                do,
                q,
                kv[:, :, 0],
...
class FlashAttnFunc(paddle.autograd.PyLayer):
    ...
    @staticmethod
    def backward(ctx, do):
        q, k, v, o, lse, bias = ctx.saved_tensor()

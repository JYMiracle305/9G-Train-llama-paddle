...
def draw_loss(title, log_files, log_parser=default_log_parser):
    ...
    plt.plot(mlu_x, mlu_y, label="bi-7b-incremental", linewidth=1)
    
    gpu_x = [i for i in range(len(logs_loss[1]))]
    gpu_y = logs_loss[1]
    plt.plot(gpu_x, gpu_y, label="gpu-7b-incremental", linewidth=1)
...
def configure_pattern():
    ...
    global mlu_file
    global gpu_file
    #mlu_file = "01-26-17-37-incremental-llama2-7B-MLU.log"
    #gpu_file = "1nodes_A100_train_7b.log"
    mlu_file = "incremental_train.4_4096_FA_bf16.log"
    gpu_file = "train_7b.log"
    #gpu_file = "sft-llama2-7B-bf16-no-offload-no-fa-GPU.log"
...
if __name__ == "__main__":
    ...
    draw_loss("7b-incremental-loss", log_files)

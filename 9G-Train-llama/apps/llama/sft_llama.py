...
sys.path.insert(0, "/home/songxin/9G-Train-llama")
...
def initialize():
    ...
    if "checkpointing" in inspect.signature(bmt.init_distributed).parameters:
        #bmt.init_distributed(checkpointing=False, seed=args.seed, zero_level=1)
        bmt.init_distributed(checkpointing=False, seed=args.seed)
    else:
        #bmt.init_distributed(seed=args.seed, zero_level=1)
...
def finetune(
    ...
    for epoch in range(args.epoch):
        ...
        for iteration, data in enumerate(dataloader):
            ...
            #bmt.print_rank("###*****************************************")
            #bmt.print_rank(data["inputs"].shape[0])
            #bmt.print_rank(args.batch_size)
            #bmt.print_rank("###*****************************************")
